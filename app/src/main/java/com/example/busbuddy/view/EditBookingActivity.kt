...
class EditBookingActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            EditBookingScreen(bookingId)
        }
    }
}

@Composable
fun EditBookingScreen(bookingId: String) {
    val context = LocalContext.current
    val activity = context as? Activity
    val viewModel = remember { BookingViewModel() }

    // Dummy values for the UI. In a real app, you would load these values from Firebase
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Button(onClick = {
            ...
            viewModel.updateBooking(updatedBooking) { success, message ->
